---
timestamp: 'Thu Oct 30 2025 00:21:44 GMT-0400 (Eastern Daylight Time)'
parent: '[[../20251030_002144.fefdf886.md]]'
content_id: 09977e3097745fd84992a341e60c0a7af4a67b63f0eb0a14265c377f6b4e4f9b
---
...
import { testDb } from "@utils/database.ts"; // Keep this for consistency, even if not directly used by the in-memory ConceptGenerator
...
Deno.test("ConceptGenerator functionality", async (t) => {
    ...
    // This is the correct placement for t.afterAll when dealing with resources opened for the entire Deno.test block.
    t.afterAll(async () => {
        await client.close();
    ...
    // we will initialize ConceptGenerator and MockLLM at the beginning of each t.step function.
    ...
    await t.step("updateInput action: updates current files", () => {
        ...
        const mockLLM = new MockLLM(); // Fresh instance for this test step (though not used in this specific step)
    ...
    await t.step("generate action: successfully generates a draft for a valid question", async () => {
        ...
        mockLLM.setQuestionValidation(true); // Default: questions are valid
        mockLLM.setFeedbackValidation(true); // Default: feedback is valid
        mockLLM.setDefaultResponse("LLM generated content"); // Default content for generation/regeneration
        mockLLM.setEditFeedback(["Minor style adjustments"]); // Default inferred feedback for edits
    ...
    await t.step("feedback action: fails if draft is accepted (requires condition)", async () => {
        ...
        const mockLLM = new MockLLM(); // Fresh instance
        mockLLM.setQuestionValidation(true);
LLM.setFeedbackValidation(true);
        mockLLM.setDefaultResponse("LLM generated content");
        mockLLM.setEditFeedback(["Minor style adjustments"]);
